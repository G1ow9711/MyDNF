...
## Task 175 Iron Forge Vanguard Real-Control Counter Slice
- Two parallel audits found the next strict gaps: Liuli Furnace needs a fresh-new-save staged route rather than a pre-unlocked shortcut, and real browser combat evidence was missing for Ink/Iron player skills. This slice addresses the highest-value Iron gap without claiming Liuli completion.
- Real browser acceptance now seeds only legal advancement requirements, clicks `iron-forge-guardian` and `black-furnace-vanguard`, enters through the focused town button, and uses `A`, `S`, and `Space` to verify Iron Palm model-following windup/impact VFX, Anvil Guard status VFX, and Black Furnace Aegis status VFX. It then moves into live range and observes actual monster skill VFX.
- Shield hits now preserve the configured guard window, prevent knockback/hurt-lock/skill cancellation, but still apply reduced damage, hitstop, enemy feedback, and guard resource gain. This makes the defensive class usable against overlapping monster attacks instead of losing every action after the first hit.
- Verification: focused real browser test passed (1 passed, 19 skipped); serial combat plus app integration regression passed (363 tests); `npm run build`, diff check, and HTTP 200 passed.

## Task 176 Fresh-Save Liuli Campaign and Browser Click Reliability
...
- Confirmed a clean zero-divergence branch at `15143a2`. Started a no-subagent audit of the story baseline, quest state machine, mounted quest panel, keyboard routing, and browser acceptance paths.
- Selected runtime NPC briefing/turn-in dialogue because the current seven-quest progression is functional but directly claims rewards from a single button, contradicting the design requirement for short skippable portrait-backed story scenes.
- Generated and integrated an original five-character Chinese fantasy-industrial NPC portrait atlas. Added seven data-driven quest scripts with three-line briefing and turn-in phases for the guild archivist, forge smith, rescued trader, liuli appraiser, and costume keeper.
- Added runtime-only story-dialogue state, valid active/ready guards, final-line and skip settlement through `claimQuestReward()`, full-screen portrait/environment presentation, per-quest mounted entries, real mouse routing, and Enter/Space/Escape control.
...
- Started from clean pushed commit `1ac2fc6` on `feature/vertical-slice`.
- Parallel read-only Ink, Iron, and acceptance audits were started after the user's explicit subagent approval. Both class audits confirmed the visible fallback is caused by a two-class runtime whitelist, hard-coded Ember atlas identity, and CSS hiding unsupported sprites.
- Generated original square 4x4 chroma-key atlases for Ink Shadow Ranger and Iron Forge Guardian. Each atlas contains real silhouette changes for idle, run, class attacks, hit recoil, launch/fall, and downed states; integration and alpha validation follow.
- Removed both chroma backgrounds into 1254x1254 RGBA assets. Alpha validation found fully transparent corners and non-empty silhouettes in every one of the sixteen cells for both classes; visual inspection found no rectangular background.
- Task217 focused RED added exact manifest, source/load, CSS isolation, numeric-facing, class identity, and action-frame assertions. Before production integration it failed all three new behavioral boundaries while 95 existing UI tests stayed green.
- Added all-four-class atlas preloading, exact class-id background mapping, numeric/text facing support, received-hit/grab priority, Ink shot/net poses, Iron shield/grab/slam poses, and shared jump/landing/dodge frame selection.
- Mounted atlas identity now follows the selected class. When ready, both new classes hide the whole portrait, baked-duplicate weapon layer, and portrait trail; class-specific filters keep silhouettes grounded in the scene.
- Focused UI GREEN passes 98/98, related app/render/weapon regressions pass 180/180, and the production TypeScript/Vite build passes.
- Real Edge acceptance initially exposed two test-timing problems and one real presentation problem: the recorder expired after natural enemy recovery, idle-only waits raced natural attacks, and wide new poses clipped at the left edge on 750px viewports. Recorder restart/action-frame proof and class-specific responsive anchors fixed the route without changing combat state or enemy timing.
- Final focused Edge rerun passes both classes. Inspected 1440x900 action and 750x485 narrow captures show visible transparent characters, baked weapons following the body pose, five live monsters, no duplicate legacy layer, and no rectangular portrait background.
- Final non-browser regression passes 727/727; all 28 real-browser computed-style regressions pass; TypeScript/Vite production build and whitespace checks pass.
- Compatibility verification passes the original KOF player/monster frame route, the live Iron Palm trash-grab/Boss-resistance route, and both new Ink/Iron dedicated-atlas routes. The KOF rerun also corrected its stale mismatched key release and inverted bottom-control geometry assertion.
- Final main-path review preserved all four class identities, kept enemy timelines unchanged, and added facing-aware Iron grab keyframes. The additional real-browser Iron Palm computed-style rerun passes.
- Task 217 is ready for its Chinese commit and push on `feature/vertical-slice`; the overall game goal remains active beyond this bounded visual-combat slice.
