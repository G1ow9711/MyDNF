...
import { createCombatSpriteStage } from "./combat-sprite-stage";
...
export type AppAction =
  ...
  | { type: "combatSkillChargeStart"; skillId: "meteor-knuckle"; inputMethod?: CombatSkillInputMethod }
  | { type: "combatSkillChargeRelease"; skillId: "meteor-knuckle" }
...
function enemyMotion(
  ...
  if (elapsedMs < (enemy.grabbedUntilMs ?? 0)) {
    return "grabbed";
  }

  if (elapsedMs < (enemy.grabThrowUntilMs ?? 0)) {
    return "grab-throw";
  }

  if (elapsedMs < (enemy.wallBounceUntilMs ?? 0)) {
    return "wall-bounce";
  }
...
function renderCombatVfx(run: CombatRun): string {
  ...
  const hitVfx = hits
    .map((hitEvent, hitIndex) => {
      ...
      const wallBounceClass = hitEvent.wallBounce ? " is-wall-bounce" : "";
      const grabClass = hitEvent.grabResult ? ` is-grab-${hitEvent.grabResult}` : "";
      const grabLabel =
        hitEvent.grabResult === "caught"
          ? "擒拿"
          : hitEvent.grabResult === "resisted"
            ? "抓取抵抗"
            : hitEvent.grabResult === "thrown"
              ? "投掷"
              : "";
      const damageLabels = [
        ...
        hitEvent.otgHit ? "扫地" : "",
        hitEvent.wallBounce ? "撞墙" : ""
      ]
        .filter(Boolean)
        .join(" ");
      const damageDisplayLabels = [damageLabels, grabLabel].filter(Boolean).join(" ");

      return `
        ${skillImpactVfx}
        <div class="hit-impact hit-impact-${hitEvent.action ?? "test"}${airImpactClass}${dashImpactClass}${groundLightImpactClass}${criticalClass}${backAttackClass}${counterHitClass}${juggleProtectedClass}${otgHitClass}${wallBounceClass}${grabClass}" data-impact-spark="true" data-grab-result="${hitEvent.grabResult ?? ""}" data-critical="${hitEvent.critical ? "true" : "false"}" data-back-attack="${hitEvent.backAttack ? "true" : "false"}" data-counter-hit="${hitEvent.counterHit ? "true" : "false"}" data-positional-multiplier="${hitEvent.positionalMultiplier}" data-juggle-count="${hitEvent.juggleCount}" data-juggle-protected="${hitEvent.juggleProtected ? "true" : "false"}" data-otg-hit="${hitEvent.otgHit ? "true" : "false"}" data-wall-bounce="${hitEvent.wallBounce ? "true" : "false"}" data-wall-bounce-side="${hitEvent.wallBounceSide ?? ""}" data-hit-event-id="${hitEvent.id}" data-vfx-action="${hitEvent.action ?? "test"}" data-hit-phase="${hitEvent.hitPhase ?? ""}" data-vfx-cue="${hitEvent.vfxCue ?? ""}" data-impact-air-action="${airImpactAction}" data-impact-dash-action="${dashImpactAction}" data-impact-ground-light-step="${groundLightImpactStep}" data-impact-origin-x="${Math.round(impactPosition.x)}" data-impact-origin-y="${Math.round(impactPosition.y)}" data-hitstop-ms="${hitEvent.hitstopMs}" style="${combatActorStyle(run, impactPosition.x, impactPosition.y)}">
          <span class="hit-ring"></span>
          <span class="hit-slash"></span>
        </div>
        <div class="damage-number${criticalClass}${backAttackClass}${counterHitClass}${juggleProtectedClass}${otgHitClass}${wallBounceClass}${grabClass}" data-damage-number="true" data-grab-result="${hitEvent.grabResult ?? ""}" data-critical="${hitEvent.critical ? "true" : "false"}" data-back-attack="${hitEvent.backAttack ? "true" : "false"}" data-counter-hit="${hitEvent.counterHit ? "true" : "false"}" data-positional-multiplier="${hitEvent.positionalMultiplier}" data-juggle-count="${hitEvent.juggleCount}" data-juggle-protected="${hitEvent.juggleProtected ? "true" : "false"}" data-otg-hit="${hitEvent.otgHit ? "true" : "false"}" data-wall-bounce="${hitEvent.wallBounce ? "true" : "false"}" data-wall-bounce-side="${hitEvent.wallBounceSide ?? ""}" data-hit-event-id="${hitEvent.id}" data-damage-origin-x="${Math.round(impactPosition.x)}" data-damage-origin-y="${Math.round(impactPosition.y)}" style="${combatActorStyle(run, impactPosition.x, impactPosition.y)}">${damageDisplayLabels ? `${damageDisplayLabels} ` : ""}${hitEvent.damage > 0 ? `-${hitEvent.damage}` : ""}</div>
      `;
    })
    .join("");
  const skillVfx =
...
function renderCombatActors(run: CombatRun, state: GameState): string {
  ...
  const latestGrabHit = [...recentHits].reverse().find((event) => event.grabResult !== undefined);
  ...
  const playerChargeTier = playerChargeProgress >= 1 ? "maximum" : playerChargeProgress >= 0.4 ? "charged" : "quick";
  const playerGrabPhase =
    run.player.activeGrabTargetId && run.elapsedMs < (run.player.activeGrabUntilMs ?? 0)
      ? "hold"
      : latestGrabHit?.grabResult === "thrown"
        ? "release"
        : latestGrabHit?.grabResult === "resisted"
          ? "resist"
          : "none";
  ...
  const enemyActors = run.enemies
    .filter(
      (enemy) =>
        enemy.hp > 0 ||
        enemy.defeatedAtMs === undefined ||
        run.elapsedMs - enemy.defeatedAtMs < enemyDeathVisibleMs
    )
    .map((enemy) => {
      const enemyState = enemy.hp > 0 ? "alive" : "defeated";
      const deathAgeMs = enemy.defeatedAtMs === undefined ? 0 : Math.max(0, run.elapsedMs - enemy.defeatedAtMs);
      const deathProgress = Math.min(1, deathAgeMs / enemyDeathVisibleMs);
      const deathPhase =
        enemy.hp > 0
          ? "alive"
          : deathAgeMs < 180
      ...
      const enemyGrabPhase =
        run.elapsedMs < (enemy.grabbedUntilMs ?? 0)
          ? "held"
          : run.elapsedMs < (enemy.grabThrowUntilMs ?? 0)
            ? "thrown"
            : "none";
...
function renderCombatScene(run: CombatRun, state: GameState): string {
  ...
  const criticalChance = Math.round(run.combatProfile.criticalChance);
  ...
  return `
    <section class="combat-scene" aria-label="战斗 · ${combatDifficulty.displayName}" data-combat-difficulty="${run.difficultyId}" data-combat-objective="${objective}" data-dungeon-id="${run.dungeonId}" data-room-index="${run.roomIndex}" data-room-count="${roomCount}" data-combat-elapsed-ms="${run.elapsedMs}" data-combat-event-count="${run.events.length}" data-combat-loot-event-count="${run.lootEvents.length}" data-floor-loot-count="${run.floorLoot ? 1 : 0}" data-live-enemy-count="${liveEnemyCount}" data-defeated-enemy-count="${defeatedEnemyCount}" data-player-hp="${run.player.hp}" data-player-max-hp="${run.player.maxHp}" data-player-x="${Math.round(run.player.x)}" data-player-y="${Math.round(run.player.y)}" data-combat-camera-x="${cameraX}" data-combat-camera-state="${camera.state}" data-combat-camera-viewport-width="${combatCameraViewportWidth}" data-combat-camera-world-width="${run.arena.width}" data-gate-enter-ready="${gateEnterReady ? "true" : "false"}" data-class-id="${state.player.classId}" data-advancement-id="${state.player.advancementId ?? ""}" data-resource-id="${run.player.resource.id}" data-resource-current="${run.player.resource.current}" data-resource-max="${run.player.resource.max}" data-combo-count="${run.comboCount}" data-critical-chance="${criticalChance}" data-critical-accumulator="${Math.round(run.criticalAccumulator * 100) / 100}" data-critical-hit="${sceneHit?.critical ? "true" : "false"}" data-room-gate-state="${roomGate.state}" data-room-gate-target-room="${roomGate.targetRoomIndex ?? ""}" data-room-transition-state="${run.roomTransition?.state ?? "none"}" data-room-transition-from-room="${transitionFromRoom}" data-room-transition-target-room="${transitionTargetRoom}" data-room-transition-gate-state="${transitionGateState}" data-room-transition-progress="${roomTransitionProgress || ""}" data-screen-shake="${sceneScreenShake}" data-screen-flash="${sceneScreenFlash}" data-hitstop-active="${sceneHitstopActive ? "true" : "false"}" data-impact-skill-id="${sceneImpactSkillId}" data-action-buffer-state="${bufferState}" data-buffered-action="${bufferedActionName(bufferedAction)}" data-buffered-skill-id="${bufferedSkillId(bufferedAction)}" data-buffered-execute-at-ms="${bufferExecuteAtMs ?? ""}" data-buffer-ms-remaining="${bufferRemainingMs}" data-buffer-window-ms="${actionBufferWindowMs}" data-cancel-source="${run.player.cancelSource}" data-cancel-window-started-at-ms="${run.player.cancelWindowStartedAtMs || ""}" data-cancel-window-until-ms="${run.player.cancelWindowUntilMs || ""}" data-cancel-lock-state="${cancelLockedSkillIds.length > 0 ? "active" : "none"}" data-cancel-lock-ms-remaining="${cancelLockRemainingMs}" data-cancel-route-skill-ids="${cancelRouteSkillIds.join(",")}" data-combo-cancel-window-active="${comboCancelWindow ? "true" : "false"}" data-combo-cancel-available="${comboCancelAvailable ? "true" : "false"}" data-combo-cancel-state="${comboCancelState}" data-combo-cancel-active="${comboCancelCast ? "true" : "false"}" data-combo-cancel-skill-id="${comboCancelCast?.skillId ?? ""}" data-combo-cancel-ms-remaining="${comboCancelWindow ? Math.max(0, run.player.cancelWindowUntilMs - run.elapsedMs) : 0}" data-kiln-shadow-haste-active="${kilnShadowHasteActive ? "true" : "false"}" data-kiln-shadow-haste-started-at-ms="${run.player.kilnShadowHasteStartedAtMs || ""}" data-kiln-shadow-haste-until-ms="${run.player.kilnShadowHasteUntilMs || ""}" data-kiln-shadow-haste-ms-remaining="${kilnShadowHasteRemainingMs}" data-boss-phase="${bossPhase}" data-boss-phase-triggered="${bossPhaseTriggered ? "true" : "false"}" data-arena-danger="${arenaDanger}" data-arena-hazard-count="${arenaHazardCount}" data-command-release-source="${commandReleaseSource}" data-command-match-skill-id="${commandReductionApplied ? latestCast?.skillId ?? "" : ""}" data-command-reduction-applied="${commandReductionApplied ? "true" : "false"}" data-last-input-method="${latestCast?.inputMethod ?? (latestCast ? "hotkey" : "none")}">
      ...
      <div class="combat-status">
        <p>${combatDifficulty.displayName} · 房间 ${run.roomIndex + 1} · HP ${run.player.hp}/${run.player.maxHp} · ${run.player.resource.displayName} ${run.player.resource.current}/${run.player.resource.max} · 连段 ${run.player.comboStep} · 攻击 ${attackValue} · 防御 ${defenseValue} · 暴击 ${criticalChance}% · 冷却 ${cooldownValue}%</p>
        <ul>${enemies}</ul>
      </div>
...
function renderDungeonResult(result: DungeonClearResult, state: GameState): string {
  ...
  const rechallengeEntry = canEnterDungeon(state, result.dungeonId, result.difficultyId);
  const fatigueCurrent = state.player.fatigue.current;
  const fatigueAfterRetry = fatigueCurrent - difficulty.fatigueCost;
  ...
  return `
    <section class="dungeon-result" data-dungeon-result="true" data-result-dungeon-id="${result.dungeonId}" data-result-difficulty-id="${result.difficultyId}" data-result-rank="${result.rank}" data-result-score="${result.score}" data-result-max-combo="${result.stats.maxCombo}" data-result-accuracy="${result.stats.accuracyPercent}" data-result-critical-hits="${result.stats.criticalHits}" data-result-hits-taken="${result.stats.hitsTaken}" data-result-clear-time-ms="${result.stats.clearTimeMs}" data-result-damage-dealt="${result.stats.damageDealt}" data-result-gear-id="${gear?.id ?? ""}" data-result-rank-bonus-gold="${result.rankBonus.gold}" data-result-rank-bonus-iron-dust="${result.rankBonus.ironDust}" data-rechallenge-available="${rechallengeEntry.canEnter ? "true" : "false"}" data-result-fatigue-current="${fatigueCurrent}" data-result-fatigue-cost="${difficulty.fatigueCost}" data-result-fatigue-after-retry="${fatigueAfterRetry}" aria-label="地下城通关结算">
      ...
      <div class="dungeon-result-content">
        ...
        <div class="dungeon-result-actions">
          <div class="dungeon-result-fatigue">
            <span>疲劳 ${fatigueCurrent}/${state.player.fatigue.max}</span>
            <strong>再次挑战消耗 ${difficulty.fatigueCost}</strong>
            ${rechallengeEntry.canEnter ? `<small>挑战后剩余 ${fatigueAfterRetry}</small>` : `<small data-rechallenge-reason="${rechallengeEntry.reason}">疲劳值不足，无法再次挑战</small>`}
          </div>
          <div class="dungeon-result-buttons">
            <button class="dungeon-result-rechallenge" data-rechallenge-dungeon="true"${rechallengeEntry.canEnter ? "" : " disabled"}>再次挑战 <span>快捷键 R</span></button>
...
function combatEventSfxIds(event: CombatEvent, run: CombatRun): string[] {
  ...
  if (event.kind === "hit") {
    ...
    return [
      ...
      event.wallBounce ? "wall-bounce-confirm" : undefined,
      event.grabResult === "caught" ? "grab-catch-confirm" : undefined,
      event.grabResult === "thrown" ? "grab-throw-impact" : undefined,
      event.grabResult === "resisted" ? "grab-resist-confirm" : undefined
    ].filter(
  ...
  if (event.kind === "skill-charge") {
    if (event.phase === "start") {
      return ["meteor-charge-start"];
    }

    return [event.chargeTier === "maximum" ? "meteor-charge-maximum" : "meteor-charge-release"];
  }
...
export function reduceAppAction(model: AppModel, action: AppAction): AppModel {
  switch (action.type) {
    case "rechallengeDungeon": {
      if (model.mode !== "dungeon-result" || !model.dungeonResult) {
        return model;
      }

      const { dungeonId, difficultyId } = model.dungeonResult;
      const entry = canEnterDungeon(model.state, dungeonId, difficultyId);
      ...
      const state = consumeDungeonEntry(model.state, dungeonId, difficultyId);
      ...
      return {
        ...
        combatRun: createCombatRun(state, dungeonId, difficultyId),
        dungeonResult: undefined,
        lastLoot: undefined,
        message: undefined,
        audio: playBgm(model.audio, chooseMusicLayer({ mode: "dungeon", dungeonId, danger: 0.2 }).trackId)
      };
    }
    ...
    case "combatTick": {
      ...
      const deathVisualPending =
        roomCleared &&
        model.combatRun.enemies.some(
          (enemy) =>
            enemy.hp <= 0 &&
            enemy.defeatedAtMs !== undefined &&
            model.combatRun!.elapsedMs - enemy.defeatedAtMs < enemyDeathVisibleMs
        );
      const tickInput = {
      ...
      if (
        (roomCleared && !deathVisualPending && !model.combatRun.roomTransition && !hasTickMovement) ||
        model.combatRun.failed ||
        model.combatRun.player.defeated
      ) {
        return model;
      }
    ...
    case "combatSkillChargeRelease": {
      ...
      const combatRun = releaseMeteorKnuckleCharge(model.combatRun);
      return {
        ...model,
        combatRun,
        message: undefined,
        audio: enqueueNewCombatEventSfx(model.audio, model.combatRun, combatRun)
      };
    }
    case "combatAction": {
      if (!model.combatRun) {
...
export function mountApp(root: HTMLDivElement): () => void {
  ...
  let activeMeteorChargeKeyCode: "KeyH" | "KeyO" | undefined;
  ...
  function render(): void {
    if (model.combatRun?.player.activeChargeSkillId !== "meteor-knuckle") {
      activeMeteorChargeKeyCode = undefined;
    }
  ...
  if ("addEventListener" in root) {
    ...
    root.addEventListener("click", (event) => {
      ...
      if (target.dataset.rechallengeDungeon) {
        dispatch({ type: "rechallengeDungeon" });
      }
    ...
    const keydownHandler = (event: KeyboardEvent) => {
      ...
      event.preventDefault();
      if (action.type === "combatAction" && action.action === "skill" && action.skillId === "meteor-knuckle") {
        if (!isRepeat) {
          activeMeteorChargeKeyCode = event.code === "KeyO" ? "KeyO" : "KeyH";
          dispatch({ type: "combatSkillChargeStart", skillId: "meteor-knuckle", inputMethod: "hotkey" });
        }
        render();
        return;
      }
    ...
    const keyupHandler = (event: KeyboardEvent) => {
      if (
        ...
        event.preventDefault();
        activeMeteorChargeKeyCode = undefined;
        dispatch({ type: "combatSkillChargeRelease", skillId: "meteor-knuckle" });
        render();
        return;
      }
