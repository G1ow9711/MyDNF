...
- Current next-candidate note: read-only UI audit found enemy skill-specific classes are currently stored as `data-enemy-skill-motion-class` rather than actual classes; future work should either make that a real class or rewrite tests around the true CSS hook and browser computed animation.
