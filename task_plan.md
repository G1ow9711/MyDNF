...
- Current monster-skill feedback progress: `ash-ember-spit`, `zheng-shockwave`, and `taotie-flame-breath` now render skill-specific target feedback classes and dedicated VFX plus hit/miss feedback animations; boss breath also has tick-specific sustain trails and telegraph animations, and `taotie-forge-shackle` bind/slam VFX now resolve from exact cue selectors with browser computed-style verification.
