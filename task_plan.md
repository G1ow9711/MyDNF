...
- [x] Audit room populations, spawn geometry, multi-target hitboxes, pursuit, and simultaneous enemy attacks.
- [x] Lock five-enemy normal rooms, four-enemy elite rooms, authored lane formations, and a two-slot attack director.
- [x] Add RED room composition, formation, attack-slot, rendering, and real-browser crowd coverage.
- [x] Implement crowd compositions and fair attack arbitration without weakening enemy actions.
...
- Task 208's four-route browser regression passed first-room clear/camera, two-room Boss entry, and full Cinder clear, but the default Cinder-to-Liuli campaign died in Liuli elite room 3 after 77 seconds with two enemies alive. The keyboard clear helper never used its three healing potions and relied only on post-death revival. It now consumes a real `Digit1` potion at low HP before continuing attacks; enemy HP and damage remain unchanged.
- Task 208's first depth-sort RED patch assumed the camera smoke test asserted a literal `.combat-world {` token. That exact assertion is not present, so the multi-file patch was rejected atomically; depth markup and CSS checks are being inserted at verified local anchors.
- Task 208's second crowd-browser run waited for background decode before dashing farther to X400. That consumed the safe opener, two enemies began attacks, and the real `KeyG` input was rejected during hurt lock. The route now restores the proven X340 opener and waits for image decode only after hit evidence, before screenshot capture.
- Task 208's first formation patch used a broad `return` anchor and inserted the formation block into `playerSkillMovementPosition()` while changing `createEnemy()` to reference it. Inspection caught the out-of-scope reference immediately; the block was removed and reinserted after `createEnemy()`'s attack-pattern calculation before any test or build ran.
- Task 208's first active-attack markup patch tried to match a substring as if it were a complete patch line, so only the preceding combined patch was rejected. The count is now exposed on the short `render-layer-count` element for stable browser queries.
- Task 208's first combined production patch matched an abbreviated combat-scene tag that differs from the real one-line template. The entire patch was rejected atomically; core, computed state, and attribute edits were then applied separately.
- Task 208's first combined RED patch used a mojibake assertion as an app-test context line, which did not match the file's actual encoded bytes. The patch was rejected atomically with no changes; split patches now use ASCII test-title anchors only.
- Task 208's first exact-count `rtk rg` command used a PowerShell-escaped alternation that RTK parsed as an unclosed regular expression. No files changed; subsequent searches use separate simple patterns.
