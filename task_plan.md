...
- Current DNF air-combat progress: `X/J` during the airborne jump window now triggers a one-per-jump air-light attack instead of being dropped or buffered to landing. The air hit uses a real 65 ms delayed hit frame, target recheck, short movement/facing freeze for model-hitbox alignment, cancel-on-interrupt/landing guards, `air-light` player motion, weapon motion, monster hit reaction, and `air-light-slash` impact VFX.
